...
class EngineConfig:
    ...
    # Velocity
    VELOCITY_THRESHOLD = 0.1    # Low-velocity signal (informational, not gating)
...
def _compute_transfer_targets(
    ...
    """
    For each product × source facility, find the best transfer target:
        - different facility
        - higher daily velocity at target
        - target can absorb full qty before its own expiry
        - transfer_value = qty * unit_cost - logistics_cost > 0

    Returns a DataFrame with columns:
        product_id, facility_id, target_facility_id,
        transfer_km, transfer_cost, transfer_value
    """
    records = []
    ...
    for row in df.iter_rows(named=True):
        ...
        for (s, t), km in config.BRANCH_DISTANCES_KM.items():
            ...
            tgt_vel = matches["daily_velocity"][0]
            tgt_dte = matches["days_to_expiry"][0]
            ...
            if (tgt_vel * days_usable) < row["qty_at_risk"]:
                continue  # target cannot absorb the stock

            logistics = (
                config.TRANSFER_BASE_COST
                + km * config.TRANSFER_COST_PER_KM * row["qty_at_risk"]
            )
            value = row["qty_at_risk"] * row["unit_cost"] - logistics

            records.append({
                "product_id":         row["product_id"],
                "facility_id":        src,
                "target_facility_id": t,
                "transfer_km":        km,
                "transfer_cost":      round(logistics, 2),
    ...
    df_candidates = pl.DataFrame(records)

    # Keep best transfer per product × source
    return (
        df_candidates
        .sort("transfer_value", descending=True)
        .unique(subset=["product_id", "facility_id"], keep="first")
    )
...
def apply_recommendation_logic(
    ...
    df = df.with_columns([
        ...
        (
            pl.col("effective_M") *
            (
                (pl.lit(float(config.DISCOUNT_WINDOW)) - pl.col("days_to_expiry"))
    ...
    if df_geo is not None and "facility_id" in df_geo.columns:
        ...
        # High local competition → demand for discounts is stronger
        if "High_Pressure_Comp" in df.columns:
            df = df.with_columns([
                pl.when(pl.col("High_Pressure_Comp") > pl.lit(3))
                .then(pl.col("discount_value") * pl.lit(1.10))
                .otherwise(pl.col("discount_value"))
                .alias("discount_value")
            ])

        # Lifestyle-driven branch → bundle appeal is stronger
        if "Lifestyle_Drivers" in df.columns:
            df = df.with_columns([
                pl.when(pl.col("Lifestyle_Drivers") > pl.lit(10))
                .then(pl.col("bundle_value") * pl.lit(1.10))
