...
import kakamega.revenue_module.data_layer as dl
import kakamega.revenue_module.predictive as predictive
import kakamega.revenue_module.whatif as whatif
...
st.set_page_config(
    page_title=f"kakamega · {PAGE_TITLE}",
    layout="wide",
...
with st.sidebar:
    try:
        st.image("assets/kakamega_logo.png", width=160)
    except Exception:
        st.markdown(
            '<div style="font-size:16px;font-weight:800;color:#0072CE;'
            'padding:8px 0 16px">kakamega</div>',
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        ...
        'Schema: <span style="color:#003467;font-weight:600">kakamega_PROD</span></div>',
        unsafe_allow_html=True,
    )
...
st.markdown(
    ...
    'kakamega · Revenue Intelligence · Powered by Snowflake'
    '</div>',
    unsafe_allow_html=True,
)